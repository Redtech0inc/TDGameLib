...
---@return table|nil image the image as Matrix made of color values or nil if directory does not exist
...
function gameLib:addHologram(lvl,text,textColor,textBackgroundColor,priority,x,y,dynamic,wrapped)
    ...
    node[keys[#keys]] = {type="hologram", dynamic=dynamic, text=textOut, textMaxWidth=textMaxWidth, textColor=textColor, textBackgroundColor=textBackgroundColor, x=math.floor(x), y=math.floor(y), elementNum=elementNum, wrapped = wrapped}
    if dynamic then
...
---@param wrapped boolean|nil if false won't wrap the text when to big (smart wrapping: wraps at last space if there is one in the current line otherwise wraps to screen size). Won't change if not supplied
...
function gameLib:cloneObject(lvl,priority,x,y,groupClones)
    ...
    if node.type == "sprite" then
        ...
        node[keys[#keys]]={type="spriteClone", sprite=lvl, x=math.floor(x), y=math.floor(y), elementNum=#self.gameMEM.objects.render.list.sprites+1, isGrouped=groupClones}
    ...
    elseif node.type == "hologram" then
        ...
        node[keys[#keys]] = {type="hologramClone", dynamic=dynamic, text=lvl, textMaxWidth=textMaxWidth, x=math.floor(x), y=math.floor(y), elementNum=elementNum, wrapped = wrapped, isGrouped=groupClones}
        if dynamic then
...
---@param lvlTable table is a table of object (= sprites, holograms) hierarchies that are a part of this group e.g: "test.string","test.number",ect...
...
---@param groupLvl string is a string that gives it the hierarchy e.g: "test.string"
---@param lvlTable table is a table of object (= sprites, holograms) hierarchies that get added to this group e.g: "test.string","test.number",ect...
...
---lets you change common data of objects (=sprites, holograms)
---@param groupLvl string is a string that gives it the hierarchy e.g: "test.string"
...
function gameLib:isColliding(lvl, lvl2, isTransparent)
    ...
    if obj1.type == "sprite" or obj1.type == "spriteClone" then
        ...
        if obj1.type == "spriteClone" then
            ...
            w1, h1 = #node.sprite, table.maxn(node.sprite[1])
        else
...
---@param xIn integer is the X coordinate for the collision check
---@param yIn integer is the Y coordinate for the collision check
...
function gameLib:render()
    ...
    for i=1,#self.gameMEM.dataFileCache.clones do
        if self.gameMEM.dataFileCache.clones[i] then
            if self:cloneObject(self.gameMEM.dataFileCache.clones[i][1],self.gameMEM.dataFileCache.clones[i][2],self.gameMEM.dataFileCache.clones[i][3],self.gameMEM.dataFileCache.clones[i][4],self.gameMEM.dataFileCache.clones[i][5]) then
